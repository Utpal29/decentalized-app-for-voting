...
class Blockchain:
  ...
  def valid_chain(self, chain):
    ...
    while current_index < len(chain):
      ...
      if block['previous_hash'] != self.hash(last_block):
        return False
      if not self.valid_proof(last_block['proof'], block['proof'], self.hash(last_block)):  # type: ignore
        return False
      last_block = block 
      current_index += 1 
    return True
  ...
  def new_block(self, proof, previous_hash=None):
    ...
    # uuid is the Universal Unique Identifier, generated randomly # which is a session key, vary from each session of poll.
